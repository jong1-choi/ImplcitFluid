...
void convserveMass( glm::vec2* v, glm::vec2* v2, float* p ) {
    ...
    for( auto y = 0; y < GRID_H; y++ ) for( auto x = 0; x < GRID_W; x++ ){
        glm::vec2 gradient = glm::vec2( (p[IX(x+1,y)]-p[IX(x-1,y)])/(2*H), (p[IX(x,y+1)]-p[IX(x,y-1)])/(2*H) );
        v2[IX(x,y)] = v[IX(x,y)] - gradient;
    }
    for( auto i = 0; i < GRID_W*GRID_H; i++)
        v[i] = v2[i];
}
...
void frame(float dt) {
    ...
    diffuse( 1, density, dt );
    advection( density, densityTemp, velocity, dt );
    diffuse( 0.1, velocity, dt );
...
//    tex.create(GRID_W,GRID_H,GL_RGB,GL_FLOAT, density );
}
